...
func newMQTTConnector(conf *Config) *MQTTConnector {
...
func (c *MQTTConnector) start() {
	...
	for resp := range c.pubCh {
		if !c.client.IsConnected() {
			logger.Println("MQTTConnector.start() got data while not connected to the broker. **discarded**")
			continue
		}
		if resp.IsError {
			logger.Println("MQTTConnector.start() data ERROR from agent manager:", string(resp.Payload))
			continue
		}
