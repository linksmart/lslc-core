...
var (
	confPath = flag.String("conf", "conf/rc-proxy.json", "Resource catalog configuration file path")
)
...
func setupRouter(config *Config) (*mux.Router, error) {
	...
	var remoteCatalogs []catalog.RemoteCatalog
	for _, cat := range config.ResourceCatalog {
		if cat.Auth != nil {
			// Setup ticket client
			ticket, err := obtainer.NewClient(cat.Auth.Provider, cat.Auth.ProviderURL, cat.Auth.Username, cat.Auth.Password, cat.Auth.ServiceID)
			if err != nil {
			...
			remoteCatalogs = append(remoteCatalogs,
				catalog.RemoteCatalog{Endpoint: cat.Endpoint, Client: catalog.NewRemoteCatalogClient(cat.Endpoint, ticket)})
			continue
		}

		remoteCatalogs = append(remoteCatalogs,
			catalog.RemoteCatalog{Endpoint: cat.Endpoint, Client: catalog.NewRemoteCatalogClient(cat.Endpoint, nil)})
	}

	storage, err := catalog.NewProxyStorage(remoteCatalogs...)
	if err != nil {
		return nil, fmt.Errorf("Failed to setup remote storage: %v", err.Error())
	}

	// Create catalog API object
	api := catalog.NewReadableCatalogAPI(
		storage,
		config.ApiLocation,
